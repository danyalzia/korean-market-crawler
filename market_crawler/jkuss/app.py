...
from market_crawler.jkuss import config
from market_crawler.jkuss.data import JkussCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "https://jkussmall.com/")

    categories = await page.query_selector_all(
        ".xans-layout-category > li:nth-of-type(n+2)"
    )
    ...
    for category in categories:
        ...
        if "CHINA" not in category_text:
            category_page_url = urljoin(page.url, category_page_url)

            subcategories = await category.query_selector_all(".sub-category ul li a")
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="#login_tab1 > div > fieldset > a",
    )
...
async def run(settings: Settings):
    ...
    async with async_playwright() as playwright:
        ...
        # ? Don't exceed 6 if custom url is being crawled otherwise the website becomes very flaky
        config.MAX_PRODUCTS_CHUNK_SIZE = min(config.MAX_PRODUCTS_CHUNK_SIZE, 6)
        await crawl_urls(
...
async def extract_soldout_text(product: Element):
    for icon in await product.query_selector_all(
        'div.box > div.promotion > span[class="soldout_txt "]'
    ):
...
async def extract_product_name(document: Document):
    query = "#detail_rnb > div.scroll_info > div.infoArea > div.headingArea > div.head_box > h2"
...
@returns_future(IndexError, error.TimeoutException)
...
async def extract_table(document: Document | PlaywrightPage):
    price2 = price3 = 0
    manufacturer = manufacturing_country = delivery_fee = brand = ""

    query = "#detail_rnb > div.scroll_info > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table > tbody"
    if not (table_tbody := await document.query_selector(query)):
        raise error.QueryNotFound("Table not found", query=query)

    ths = await table_tbody.query_selector_all(
        "tr:not(.product_price_sale_login_css.xans-record-):not(.optimum_discount_price_login_css.xans-record-) > td > .item_box > div"
    )
    tds = await table_tbody.query_selector_all(
        "tr:not(.product_price_sale_login_css.xans-record-):not(.optimum_discount_price_login_css.xans-record-) > td > .item_box > span"
    )
    ...
    if price1 := await document.text_content("#span_optimum_discount_price"):
...
async def extract_images(
    ...
    with suppress(error.PlaywrightTimeoutError):
        await page.click("#mainMenu > li > a")
