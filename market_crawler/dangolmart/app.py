...
from market_crawler.dangolmart import config
from market_crawler.dangolmart.data import DangolmartCrawlData
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="fieldset > a > img",
...
async def run(settings: Settings):
    ...
    async with async_playwright() as playwright:
        ...
        # ? Don't exceed 20 if custom url is being crawled otherwise the website becomes very flaky
        config.MAX_PRODUCTS_CHUNK_SIZE = min(config.MAX_PRODUCTS_CHUNK_SIZE, 20)
        await crawl_urls(
...
async def extract_url(
    ...
    if await page.query_selector(option2_query):
        log.warning(f"Option2 is present: <blue>{product_url}</>. Skipping it.")
    ...
    if "NOT PRESENT" in detailed_images_html_source:
        log.warning(
            f"Detail images are not present: <blue>{product_url}</>. Skipping it."
        )
        product_state.done = True
        if config.USE_PRODUCT_SAVE_STATES:
            await product_state.save()
    ...
    if options:
        for option in options:
            ...
            crawl_data = DangolmartCrawlData(
...
async def extract_data(
    ...
    match R3:
        case Ok(table):
            price2, price3, delivery_fee, model_name, quantity = table
        case Err(err):
            await visit_link(page, product_url, wait_until="networkidle")
...
async def extract_product_name(document: Document):
    query = "div.detailArea > div.infoArea > div.info > h2"
...
async def extract_table(document: Document, product_url: str):
    ...
    if not (price3 := await document.text_content(query)):
        # ? See: https://dangolmart.shop/product/%EC%9D%B4%EC%A0%9C%EC%9D%B4%ED%91%B8%EB%93%9C-%EA%B9%80%EC%B2%9C-%EC%83%A4%EC%9D%B8%EB%A8%B8%EC%8A%A4%EC%BC%93-1kg-15kg-2kg-2kg-%EC%84%A0%EB%AC%BC%EC%9A%A9-4kg-%EC%84%A0%EB%AC%BC%EC%9A%A9-%EB%A9%B4%EC%84%B8%EC%8B%A4%EC%86%8D%ED%98%95-500g-%EA%B3%B5%EA%B8%89%EC%A4%91%EB%8B%A8/903/category/25/display/1/
        ...
        try:
            ...
            raise ValueError(
                f"Couldn't convert price3 text {price3} to number"
            ) from err

    query = "span[class='delv_price_B']"
    if not (delivery_fee := await document.text_content(query)):
        # ? See: https://dangolmart.shop/product/%EC%9D%B4%EC%A0%9C%EC%9D%B4%ED%91%B8%EB%93%9C-%EA%B9%80%EC%B2%9C-%EC%83%A4%EC%9D%B8%EB%A8%B8%EC%8A%A4%EC%BC%93-1kg-15kg-2kg-2kg-%EC%84%A0%EB%AC%BC%EC%9A%A9-4kg-%EC%84%A0%EB%AC%BC%EC%9A%A9-%EB%A9%B4%EC%84%B8%EC%8B%A4%EC%86%8D%ED%98%95-500g-%EA%B3%B5%EA%B8%89%EC%A4%91%EB%8B%A8/903/category/25/display/1/
        log.warning(f"Delivery fee not found: <blue>{product_url}</>")
        delivery_fee = ""
    else:
        delivery_fee = delivery_fee.strip()
        if split := delivery_fee.split("\n"):
            delivery_fee = split[0].strip()
    ...
    query = "#optionShortcut > div.optionShortcut_area > div.guideArea > p"
    if not (quantity := await document.text_content(query)):
        raise error.QueryNotFound("Quantity not found", query=query)

    return price2, price3, delivery_fee, model_name, quantity
...
async def extract_thumbnail_image(document: Document, product_url: str):
    ...
    if not (thumbnail_image := await document.get_attribute(query, "src")):
        # ? See: http://gbmb2b.com/product/detail.html?product_no=12280&cate_no=88&display_group=1
