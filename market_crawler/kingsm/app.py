...
from market_crawler.kingsm import config
from market_crawler.kingsm.data import KingsmCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "http://kingsm.co.kr/")

    categories = await page.query_selector_all("#widenavigation > li")
    ...
    for category in categories:
        ...
        subcategories = await page.query_selector_all("#snb > ul > li > a")
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query=".login .loginbtn a",
    )
...
async def extract_soldout_text(product: Element):
    for icon in await product.query_selector_all("p.name > img"):
...
async def extract_data(
    ...
    match R4:
        case Ok(table):
            model_name, price2, delivery_fee, manufacturing_country = table
        case Err(err):
    ...
    detailed_images_html_source = R7
...
async def extract_product_name(document: Document):
    query = "#maincontents > div.xans-element-.xans-product.xans-product-detail > div.infoArea > h3"
...
async def extract_table(document: Document | PlaywrightPage):
    model_name = delivery_fee = manufacturing_country = ""
    price2 = 0

    query = "#maincontents > div.xans-product-detail > div.infoArea > div.xans-product-detaildesign > table > tbody"
    ...
    return model_name, price2, delivery_fee, manufacturing_country
...
async def extract_multiple_thumbnail_image(document: Document, product_url: str):
    query = "div.xans-product-addimage.listImg > ul > li:nth-child(n+2)"
    thumbnail_image_url2 = ""
    thumbnail_image_url3 = ""
    thumbnail_image_url4 = ""
    thumbnail_image_url5 = ""
...
@cache
def image_quries():
    return "div#prdDetail:nth-child(1) > div.cont img"
...
async def extract_images(
    ...
    query = image_quries()
    unwanted_url = "http://openhost.gfapparel.cafe24.com/web/product/common/top.jpg"
    ...
    final_urls = list(map(lambda url: urljoin(product_url, url), urls))
    if unwanted_url in final_urls:
        final_urls.remove(unwanted_url)
