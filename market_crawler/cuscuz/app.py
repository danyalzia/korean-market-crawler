...
from market_crawler.cuscuz import config
from market_crawler.cuscuz.data import CuscuzCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "https://cuscuzmall.com/")

    categories = await page.query_selector_all(
        ".leftCategory .menu-dp1 > li.xans-record-:nth-of-type(n+2)"
    )

    for category in categories:
        if (text := await category.text_content()) and "SALE" not in text:
            ...
            subcategories = await category.query_selector_all(".menu-dp2 > li > a")
...
async def extract_data(
    ...
    match R3:
        case Ok(table):
            model_name, price2, delivery_fee = table
        case Err(err):
...
async def extract_table(document: Document | PlaywrightPage):
    ...
    query = "#contents > div.xans-product-detail > div.detailArea > div.infoArea > div.xans-product > table > tbody"
    ...
    return model_name, price2, delivery_fee
