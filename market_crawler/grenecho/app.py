...
from market_crawler.grenecho import config
from market_crawler.grenecho.data import GrenechoCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "https://echogolf.co.kr/")

    categories = await page.query_selector_all(
        "#category > div.position.arial > ul > li"
    )
    ...
    for category in categories:
        ...
        subcategories = await page.query_selector_all(
            "#contents > div.inner > div.xans-element-.xans-product.xans-product-menupackage > ul > li > a"
        )
        ...
        for subcategory in subcategories:
            ...
            if not (extra_part := await subcategory.query_selector(".count")) or not (
                extra_part := await extra_part.text_content()
            ):
                continue

            subcategory_text = subcategory_text.replace(extra_part, "").strip()
...
async def extract_data(
    ...
    match R4:
        case Ok(table):
            manufacturing_country, manufacturer, price2 = table
        case Err(err):
            raise error.TableNotFound(err, url=product_url)

    match R5:
        case Ok(option):
            options, sold_out_text = option
        case Err(err):
...
async def extract_product_name(document: Document):
    query = "#contents > div.xans-product-detail > div.detailArea > div.infoArea > div.headingArea > h2"
...
async def extract_table(document: Document | PlaywrightPage):
    ...
    return manufacturing_country, manufacturer, price2
...
@cache
def extra_image_quries():
    return ".edinfo-washing .edinfo-value li img"


@cache
def extra_image_optional_quries():
    return ".washing img"


@returns_future(error.QueryNotFound, error.Base64Present)
async def extract_images(
    page: PlaywrightPage, product_url: str, query: str, optional_query: str = ""
) -> list[str]:
    if optional_query:
        elements = await page.query_selector_all(f"{query},{optional_query}")
    else:
...
async def extract_html_texts(
    page: PlaywrightPage,
) -> list[str] | str:
    query = ".edinfo-sizediv table"
    regex = compile_regex(
        r"""(border|class|scope|)(=")([a-zA-Z0-9:;\.\s\(\)\-\,]*)(")"""
    )

    for p in await page.query_selector_all(query):
        with suppress(error.PlaywrightTimeoutError):
            await p.scroll_into_view_if_needed()

    texts = [
        "<table border='1' style='border-collapse:collapse'> {} </table>".format(
            re.sub(regex, "", text)
        )
        for table_content in await page.query_selector_all(
            ".edinfo-sizediv table , .mSize table"
        )
        if (text := await table_content.inner_html())
    ]

    texts_str = [
        "<div> {} </div>".format(re.sub(regex, "", text))
        for str_content in await page.query_selector_all("div.edinfo-text , div.mList")
        if (text := await str_content.inner_html())
    ]

    texts.extend(texts_str)
...
async def extract_html(
    ...
    match await extract_images(page, product_url, query=image_quries()):
        case Ok(images):
            images = list(dict.fromkeys(images))

            # looking for extra images
            match await extract_images(
                page,
                product_url,
                query=extra_image_quries(),
                optional_query=extra_image_optional_quries(),
            ):
                case Ok(extra_images):
                    extra_images = list(dict.fromkeys(extra_images))
                    images = [req_ur for req_ur in images if req_ur not in extra_images]
            ...
            texts = await extract_html_texts(page)

            return build_detailed_images_html(images, texts, html_top, html_bottom)
...
def build_detailed_images_html(
    urls: Iterable[str], texts: Iterable[str], html_top: str, html_bottom: str
):
    # ? Remove duplicate urls (if any) but preserve insertion order
    html = "".join(f"<img src='{url}' /><br />" for url in dict.fromkeys(urls))
    text = "".join(f"{text_str}<br />" for text_str in dict.fromkeys(texts))
