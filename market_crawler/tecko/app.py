...
from market_crawler.tecko import config
from market_crawler.tecko.data import TeckoCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "http://teckomall.com/index.html")

    categories = await page.query_selector_all("#category > div.position > ul > li")

    for category in categories:
        if (
            (text := await category.text_content())
            and ("땡Q상품" not in text)
            and ("리퍼상품" not in text)
        ):
            ...
            subcategories = await page.query_selector_all("ul.menuCategory > li > a")
            ...
            for subcategory in subcategories:
                ...
                sub_subcategories = await page.query_selector_all(
                    "ul.menuCategory > li > a"
                )

                for sub_subcategory in sub_subcategories:
                    if ("스트랩" in subcategory_text) or (
                        "액세서리" in subcategory_text
                    ):
                        # ? Not all sub categories may have sub categories
                        full_subcategories.append(
                            Category(
                                f"{category_text}>{subcategory_text}".replace("()", ""),
                                url,
                            )
                        )
                        ...
                        if not (
                            sub_subcategory_page_url := await sub_subcategory.get_attribute(
                                "href"
                            )
                        ):
                            continue
                        ...
                        if not (
                            sub_subcategory_text := await sub_subcategory.text_content()
                        ):
                            continue
                        ...
                        full_text = (
                            f"{category_text}>{subcategory_text}>{sub_subcategory_text}"
                        )
                        full_subcategories.append(
                            Category(full_text.replace("()", ""), sub_url)
                        )
                        full_subcategories = list(set(full_subcategories))
...
def get_productid(url: str) -> Result[str, str]:
    ...
    return Ok(f"product_id={str(match.group(0))}")
...
async def extract_data(
    ...
    match R2:
        case Ok(table):
            product_name, brand, price2, delivery_fee = table
        case Err(err):
...
async def extract_table(document: Document | PlaywrightPage):
    price2 = 0
    product_name = brand = delivery_fee = ""
    ...
    for th, td in zip(ths, tds):
        ...
        if "판매가" in heading:
            price2_str = text
            try:
                if percentage := await document.text_content("span.ec-sale-rate"):
                    price2_str = price2_str.replace(percentage, "")
    ...
    return product_name, brand, price2, delivery_fee
