...
from market_crawler.gamsungen import config
from market_crawler.gamsungen.data import GamsungenCrawlData
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query="div.xans-member-login > div.user-login > fieldset > a",
    )
...
async def extract_thumbnail_image(document: Document, product_url: str):
    query = "div.thumbnail > a > img.BigImage"
...
async def extract_html(
    ...
    match await extract_images(page, product_url, html_top, html_bottom):
        case Ok(images):
            ...
            if texts := await extract_html_texts(page):
                for text in texts:
                    images.append(text)
