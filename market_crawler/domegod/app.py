...
from market_crawler.domegod import config
from market_crawler.domegod.data import DomegodCrawlData
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query="a.btnJS.A.Login",
    )
...
def get_productid(url: str) -> Result[str, str]:
    regex = compile_regex(r"\/product\/.*\/(\d*\w*)\/category")
    return (
...
async def get_products(tree: Document | PlaywrightPage):
    ...
    if not (products := await tree.query_selector_all(query)):
        return Err(error.QueryNotFound("Products not found", query))
    return Ok(products)
...
async def extract_soldout_text(product: Element) -> str:
    for icon in await product.query_selector_all("div.iconArea > img"):
        if (alt_text := await icon.get_attribute("alt")) and "품절" in alt_text:
            return "품절"
...
async def extract_product(
    ...
    (price2, delivery_fee) = await extract_table(document)
...
async def extract_thumbnail_image(document: Document, category_page_url: str):
    query = "div.keyImg > div.thumbnail > a > img"
...
async def extract_table(document: Document):
    ...
    query = "#contents > section > div.detailArea.jsDetail > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table > tbody"
    if not (
    ...
    for key, val in zip(
        ...
        if "배송비" in key_str:
            delivery_fee_str = val_str
            try:
                delivery_fee = parse_int(delivery_fee_str)
            except ValueError:
                pass
...
@cache
def image_quries():
    return "#contents > div.xans-element-.xans-product.xans-product-additional > section.JS-detail.z1 > div > div.product > center > center > img"
...
async def extract_images(
    ...
    try:
        await page.click("div.in > div.tabArea > ul.tab.cf > li.z1.on > p")
    except error.PlaywrightTimeoutError:
        log.warning(f"Button 상품상세정보 is not present at all <blue>| {page.url}</>")
