...
from market_crawler.pettory import config
from market_crawler.pettory.data import PettoryCrawlData
...
async def get_products(document: Document):
    query = ".xans-product-listnormal .prdList > li"
...
async def extract_soldout_text(product: Element):
    for icon in await product.query_selector_all("div.promotion.cboth > img.icon_img"):
    ...
    if options:
        for option in options:
            match split_options_text(option, price3):
                case Ok((option1, _price3, option2, option3)):
                    pass
                case Err(err):
...
async def extract_data(
    ...
    match R4:
        case Ok(table):
            manufacturing_country, price2, price3, model_name = table
        case Err(err):
...
async def extract_product_name(document: Document):
    query = "#contents > div > div.detailArea > div.infoArea > div.headingArea > h2"
    return (
...
async def extract_quantity(document: Document):
    query = "#contents > div > div.detailArea > div.infoArea > div.guideArea > p"
...
async def extract_table(document: Document | PlaywrightPage):
    manufacturing_country = model_name = ""
    price2 = price3 = 0
    ...
    return manufacturing_country, price2, price3, model_name


@returns_future(error.QueryNotFound)
async def extract_thumbnail_image(document: Document, product_url: str):
    query = ".keyImg .BigImage"
...
def split_options_text(option1: str, price3: int):
    ...
    if "품절" in option1:
        return (
            option1.replace("품절", "").replace("()", "").replace("[]", ""),
            price3,
            "품절",
            option3 or "",
        )

    return option1, price3, "", option3 or ""
...
async def extract_html(
    ...
    match await extract_images(page, product_url, html_top, html_bottom):
        ...
        case Err(err):
            # ? See: https://pettory.com/product/detail.html?product_no=8215&cate_no=25&display_group=1
