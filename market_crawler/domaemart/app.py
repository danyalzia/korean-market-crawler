...
from market_crawler.domaemart import config
from market_crawler.domaemart.data import DomaemartCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "https://domaemart.com/")

    categories = await page.query_selector_all("#category-lnb > div > ul > li")
    ...
    for category in categories:
        if (text := await category.text_content()) and "커뮤니티" not in text:
            ...
            subcategories = await category.query_selector_all("div > dl > dd > a")
...
async def login_button_strategy(page: PlaywrightPage, login_button_query: str):
    ...
    # ? Annoying popup which makes it difficult to find 로그아웃 text behind it :/
    with suppress(error.PlaywrightTimeoutError):
        await page.click(
            "#d_sLightBox_win > div.inner.popup_introPopup.on > div > div.close.-cap"
        )

    await page.wait_for_selector(
        "text=로그아웃",  # 로그아웃 => Logout
        state="visible",
    )
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query="#contents > form > div > div > fieldset > a",
...
async def extract_soldout_text(product: Element):
    for icon in await product.query_selector_all(".icon .icon_img"):
...
async def extract_table(document: Document):
    model_name = manufacturing_country = manufacturer = quantity = delivery_fee = (
        message1
    ) = ""
    price2 = 0

    query = "#contents > div.xans-product-detail > div.detailArea > div.buy-wrapper > div.buy-scroll-box > div.infoArea > div.xans-product-detaildesign > table > tbody"
    ...
    for th, td in zip(ths, tds):
        ...
        if "공급가액" in heading:
            ...
            try:
                ...
                log.warning(f"Coudn't convert price2 text {price2_str} to number")
                price2 = price2_str
        ...
        if heading == "배송비":
            delivery_fee = text
            if extra_part := await document.text_content(
                ".btnTooltip.ec-front-shop-delivery-defferent-shipping"
            ):
                delivery_fee = delivery_fee.replace(extra_part, "")
                delivery_fee = "".join(delivery_fee.split()).replace(
                    "주문시결제(선결제)수령시결제(착불)", ""
                )
...
async def extract_images(
    ...
    with suppress(error.PlaywrightTimeoutError):
        await page.click(
            "#prdDetail > div.xans-element-.xans-product.xans-product-detail.tabMenu > ul > li > a"
        )
    ...
    for el in elements:
        ...
        match await do(action).retryif(
            ...
            case Ok(src):
                if src != "https://m.domaemart.com/_wg/img/img_info1.jpg":
                    urls.append(src)
