...
from market_crawler.ballys import config
from market_crawler.ballys.data import BallysCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "https://www.밸리스도매.com/product/list.html?cate_no=336")

    categories = await page.query_selector_all(
        ".sp-product--category .cate-list__sub1 > ul > li:nth-of-type(n+2)"
    )
    ...
    for category in categories:
        ...
        if category_text != "적립금 구매":
            ...
            for subcategory in subcategories:
                ...
                subcategory_text = subcategory_text.replace("()", "").strip()

                url = urljoin(
                    page.url,
                    subcategory_page_url,
                )
                full_text = f"일반 도매 회원>{category_text}>{subcategory_text}"
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query=".sp-btn-group > button.sp-btn",
    )
...
async def get_products(document: Document):
    query = ".sp-size__fixed > .sp-product__box > div"
...
async def extract_product_name(document: Document):
    query = "h1.sp-sub-product--header-h1"
    extra_part_query = ".likePrdCount"
    if not (product_name := await document.text_content(query)):
        raise error.QueryNotFound("Product name not found", query)

    if extra_part := await document.text_content(extra_part_query):
        maxreplace = 1

        product_name = "".join(product_name.rsplit(extra_part, maxreplace))
...
async def extract_table(page: PlaywrightPage):
    ...
    plus_button_query = "div.sp-toggle-base:nth-of-type(8) button"
    if await page.query_selector(plus_button_query):
        await page.click(plus_button_query)
    else:
        raise error.QueryNotFound("plus button not found", query=plus_button_query)

    query = "div.xans-element-.xans-product.xans-product-detaildesign.sp-sub-product--header-spec"
    if not (table_tbody := await page.query_selector(query)):
        raise error.QueryNotFound("Table not found", query=query)

    ths = await table_tbody.query_selector_all("ul > li:nth-child(1)")
    tds = await table_tbody.query_selector_all("ul > li:nth-child(2)")
...
@cache
def image_quries():
    return ".sp-sub-product--additional-detail img"
...
async def extract_images(
    ...
    with suppress(error.PlaywrightTimeoutError):
        await page.click(
            "#sp-sub-product > div.xans-element-.xans-product.xans-product-additional.sp-sub-product--additional > div.sp-sub-product--tabs.holder.fixed > ul > li > a:nth-child(2)"
        )
