...
from market_crawler.corna import config
from market_crawler.corna.data import CornaCrawlData
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query="button.btnLogin",
    )
...
async def extract_soldout_text(product: Element) -> str:
    for icon in await product.query_selector_all(
        "div.status > div.icon > img.icon_img"
    ):
...
async def extract_thumbnail_images(document: Document, product_url: str):
    ...
    query = "div.xans-element-.xans-product.xans-product-image.imgArea > div.xans-element-.xans-product.xans-product-addimage.listImg > ul > li > img"
    thumbnail_image_url2 = ""
    thumbnail_image_url3 = ""
    thumbnail_image_url4 = ""
    thumbnail_image_url5 = ""
...
async def extract_table(document: Document):
    product_name = model_name = manufacturer = manufacturing_country = ""
    price2 = 0

    query = "#contents > div> div.detailArea > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign.gDouble > table > tbody"
    if not (
    ...
    return product_name, model_name, price2, manufacturer, manufacturing_country
...
async def extract_images(
    ...
    try:
        await page.click("#prdDetail > ul > li:nth-child(1)")
    except:
