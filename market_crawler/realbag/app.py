...
from market_crawler.realbag import config
from market_crawler.realbag.data import RealbagCrawlData
...
async def login_button_strategy(page: PlaywrightPage, login_button_query: str) -> None:
    ...
    await page.wait_for_selector("a.top_logout")
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="div.loginform > fieldset > a > img",
...
async def get_products(document: Document):
    query = "div:not([style='display:none;']) > ul li[id^='anchorBoxId_']"
...
async def extract_soldout_text(product: Element):
    for icon in await product.query_selector_all("div > p > a > span"):
...
async def extract_product_name(document: Document):
    query = "#contents > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.infoArea > div.prdTitle > h3"
...
def split_options_text(option1: str):
    if "[일시품절]" in option1:
        return option1.replace("[일시품절]", "").strip(), "[일시품절]"
    if "[품절/단종]" in option1:
        return option1.replace("[품절/단종]", "").strip(), "[품절/단종]"
    if "[품절]" in option1:
        return option1.replace("[품절]", "").strip(), "[품절]"
    if "품절" in option1:
        return option1.replace("품절", "").strip(), "품절"
