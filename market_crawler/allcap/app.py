...
from market_crawler.allcap import config
from market_crawler.allcap.data import AllcapCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "https://allcap.co.kr/")

    categories = await page.query_selector_all(
        "#left_side > div.xans-element-.xans-layout.xans-layout-category.category > div > ul > li:nth-child(-n+4)"
    )

    for category in categories:
        if (text := await category.text_content()) and "신상품 New" not in text:
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query="li.loginbt > a > img",
    )
...
async def extract_soldout_text(product: Element):
    for icon in await product.query_selector_all("div.status > div.icon > img"):
...
async def extract_product_name(document: Document):
    query = "div.detailArea > div.infoArea > h3"
    return (
...
async def extract_table(document: Document | PlaywrightPage):
    model_name = message2 = manufacturing_country = message1 = ""
    price2 = price3 = 0

    query = "#container > div.xans-element-.xans-product > div.detailArea > div.infoArea > div.xans-element- > table > tbody"
    ...
    for th, td in zip(ths, tds):
        ...
        if "도매가" in heading:
            if not (
                price2_str := await td.query_selector("#span_product_price_sale")
            ) or not (price2_str := await price2_str.text_content()):
                raise ValueError("Price2 is not found")
        ...
        # message2
        if "출시 SIZE" in heading:
            message2 = text

    return model_name, message2, manufacturing_country, message1, price2, price3
...
async def extract_multiple_thumbnail_image(document: Document, product_url: str):
    query = "#container > div.xans-product-detail > div.detailArea > div.imgArea > div.listImg > ul > li:nth-child(n+2)"
    thumbnail_image_url2 = ""
    thumbnail_image_url3 = ""
    thumbnail_image_url4 = ""
    thumbnail_image_url5 = ""
    ...
    if thumbnail_images := await document.query_selector_all(query):
        thumbnail_images = [
            urljoin(product_url, await ti.text_content()) for ti in thumbnail_images
        ]
        N = 4
