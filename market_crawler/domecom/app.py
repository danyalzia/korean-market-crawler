...
from market_crawler.domecom import config
from market_crawler.domecom.data import DomecomCrawlData
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query="fieldset > a.btnLogin",
    )
...
async def extract_soldout(product: Element):
    if el := await product.query_selector("div.icon > div.promotion > img"):
...
async def extract_data(
    ...
    match R2:
        case Ok(Table("", _)):
            raise error.ProductNameNotFound("Product name is empty", url=product_url)
        case Ok(Table(_, 0)):
            raise error.SupplyPriceNotFound("Supply price is empty", url=product_url)
        case Ok(Table(product_name, price2)):
            pass
        case Err(err):
...
async def extract_table(tree: Document):
    ...
    ths = [
        ...
        for t in await tree.query_selector_all(
            "div.detailArea > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table > tbody > tr > th",
        )
    ...
    tds = [
        text.strip()
        for t in await tree.query_selector_all(
            "div.detailArea > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table > tbody > tr > td",
        )
        if (text := await t.text_content())
    ]
    ...
    return Table(product_name, price2)
...
async def extract_options(tree: Document):
    option1_query = "select[id='product_option_id1']"
    option1_elements = await tree.query_selector_all(
...
async def extract_images(
    ...
    # ? Heading above detail images
    query = '#prdDetail ul li a[href="#prdDetail"]'
    try:
        await page.click(query)
    except error.PlaywrightTimeoutError as err:
        raise error.QueryNotFound(
            "Product detail images heading is not found", query=query
        ) from err
