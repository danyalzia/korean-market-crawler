...
from market_crawler.todin import config
from market_crawler.todin.data import TodinCrawlData
...
def page_url(*, current_url: str, next_page_no: int) -> str:
    if ("?page" in current_url or "&page" in current_url) and (
        found := compile_regex(r"page=(\d+)&search").search(current_url)
    ):
        substr = found.group(0)
        if current_page_no := compile_regex(r"page=(\d+)&search").search(substr):
            return current_url.replace(
                f"page={current_page_no.group(1)}", f"page={next_page_no}"
            )
...
async def login_button_strategy(page: PlaywrightPage, login_button_query: str):
    # async with page.expect_navigation(): #? Strangely, for TODIN, expect_navigation() gives Timeout error (probably because it is not redirecting to another page)
    await page.click(login_button_query)
    await page.wait_for_selector(
        "text=LOGOUT",  # 로그아웃 => Logout
        state="visible",
    )
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="button[class='btn_resp size_login1']",
        keep_logged_in_check_query="#idsave",
...
async def get_products(document: Document):
    query = "li[class='goods_list_style1']"
...
async def is_prohibited_product(page: PlaywrightPage, product_url: str):
    if product_tabs := await page.query_selector(
        "#goods_tabs .goods_information_tabs > a"
    ):
        await product_tabs.click()

    try:
        await page.wait_for_selector(
            "#goods_contents_quick .goods_description img",
            timeout=50000,
        )
    except error.PlaywrightTimeoutError:
        log.warning(
            f"Product detail image element has not been found within 50000 milliseconds <blue>| {product_url}</blue>"
        )
    else:
        ...
        for image in images:
            ...
            ) and "ONLINE-INFO-2020.jpg" in src:
                log.warning(
                    f"Prohibited product detail image found, so we will not crawl the page <blue>| {product_url}</blue>"
                )
                return True
...
async def extract_product(
    ...
    # ? We want to check for the product detail images first because we don't want to crawl some products that contain the following image:
    # ? http://ai.esmplus.com/back9mall/todingolf/ONLINE-INFO-2020.jpg

    if await is_prohibited_product(page, product_url):
        await page.close()
...
async def extract_product_name(document: Document):
    query = "div.detail_title_area > h3"
...
async def extract_thumbnail_images(document: Document, product_url: str):
    query = "#goods_thumbs > div > div > img"
    if not (thumbnail_image := await document.get_attribute(query, "src")):
        raise error.QueryNotFound("Thumbnail image not found", query)

    thumbnail_image_url = urljoin(product_url, thumbnail_image)

    query = "#goods_thumbs > div.pagination_wrap ul > li> a > img"
    thumbnail_image_url2 = ""
    thumbnail_image_url3 = ""
    thumbnail_image_url4 = ""
    thumbnail_image_url5 = ""
...
async def extract_soldout_text(document: Document):
    query = "#goodsOptionBuySection > div.goods_buttons_area > ul > li p[class^='text_soldout']"
    if not (sold_out_text := await document.inner_text(query)):
        raise error.QueryNotFound("Soldout text not found", query)

    return sold_out_text.strip()
...
async def extract_table(document: Document):
    price3_text = await document.text_content(
        "li.deatil_price_area > p.sale_price > span.num"
    )
    try:
        price3 = parse_int(price3_text)
    except (ValueError, TypeError) as err:
        raise ValueError(
            f"Coudn't convert selling price text {price3_text} to number"
        ) from err

    if delivery_fee := await document.text_content(
        "ul.detail_spec_table.sub.shipping_set_area > li > div > div.std"
    ):
        delivery_fee = delivery_fee.split("(")[0]
        try:
            delivery_fee = parse_int(delivery_fee)
        except ValueError as err:
            # ? Some products have "FREE" as their Delivery Fee
            # ? See: https://www.todingolf.com/goods/view?no=1694
            delivery_fee = 0
    else:
        delivery_fee = 0

    # ? Some products don't have brand name
    # ? See: https://www.todingolf.com/goods/view?no=409
    if not (
        brand := await document.text_content(
            "li.goods_spec_brand > ul > li:nth-child(2) > a"
        )
    ):
        brand = ""
...
async def extract_options(document: Document):
    option1_query = ".goods_option_table select"
...
@cache
def image_quries():
    return "#goods_contents_quick .goods_description img"
...
async def extract_images(
    ...
    for el in elements:
        ...
        match await do(action).retryif(
            ...
            case Ok(src):
                if "http://ai.esmplus.com/back9mall/entergolf/2019.jpg" not in src:
                    urls.append(src)
