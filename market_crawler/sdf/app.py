...
from market_crawler.sdf import config
from market_crawler.sdf.data import SDFCrawlData
...
async def extract_soldout_text(product: Element):
    for detail in await product.query_selector_all(
        "div.description > ul.spec > li:nth-child(4) > span"
    ):
        text = str(await detail.text_content())
        if "품절" in text:
            return "품절"
...
async def extract_data(
    ...
    match R3:
        case Ok(table):
            price2, price3, delivery_fee, brand = table
        case Err(err):
            await page.pause()
    ...
    return ">".join(
        [
            ...
            for el in await document.query_selector_all(
                "#contents > div.xans-element-.xans-product.xans-product-headcategory.path > ol > li > a"
            )
...
async def extract_table(document: Document | PlaywrightPage):
    ...
    if not (
        table_tbody := await document.query_selector(
            query,
        )
    ):
    ...
    return price2, price3, delivery_fee, brand
