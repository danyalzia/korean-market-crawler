...
from market_crawler.sapakorea import config
from market_crawler.sapakorea.data import SapakoreaCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser, categories: list[Category]):
    ...
    for category in categories:
        ...
        for subcategory in subcategories:
            ...
            full_subcategories.append(Category(subcategory_text, url))
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="div > div > fieldset > a",
...
async def crawl(
    ...
    if not (
        el := await document.get_attribute(
            query,
            "href",
        )
    ):
...
def get_productid(url: str) -> Result[str, str]:
    ...
    if not (match := regex.search(url)) and not (
        match := compile_regex(r"\/product\/.*\/(\d*\w*)\/category").search(url)
    ):
...
async def extract_data(
    ...
    match R2:
        case Ok(table):
            (price2, price3, model_name, product_name, message1) = table
        case Err(err):
            await visit_link(page, product_url, wait_until="networkidle")
...
async def extract_table(document: Document, product_url: str):
    ...
    if not (price2 := await document.text_content(query)):
        log.warning(f"Price2 not found: {product_url}")
        price2 = ""
    else:
        try:
    ...
    product_name = model_name = message1 = ""
    ...
    return Table(price2, price3, model_name, product_name, message1)


@cache
def image_quries():
    return "#prdDetail > div > center img, #prdDetail > div img"
