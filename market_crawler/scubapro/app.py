...
from market_crawler.scubapro import config
from market_crawler.scubapro.data import ScubaproCrawlData
...
async def crawl(
    ...
    if not (document := await parse_document(await page.content(), engine="lexbor")):
        raise HTMLParsingError("Document is not parsed correctly", url=category_url)

    log.detail.page_url(category_url)

    if not (number_of_products := await has_products(document)):
        log.action.products_not_present_on_page(category_url, category_state.pageno)
...
async def load_all_products(page: PlaywrightPage):
    query = '#content > div.prod_con01 > div.w1400 > div > a[onclick="more()"]'
    clickable = True
    while clickable:
        try:
...
async def get_products(document: Document):
    query = "#content > div.prod_con01 > div > div > ul > li"
...
async def get_product_link(product: Element, category_url: str):
    if not (product_link := await product.get_attribute("onclick")):
        raise error.QueryNotFound("Product link not found", "a")

    regex = compile_regex(r"location.href=\'(.*)\'")
    return urljoin(category_url, regex.findall(product_link)[0])
...
async def extract_product(
    ...
    option3 = sold_out_text = ""

    if options:
        # ? This part is originally written by Aniqa
        # ? I modified it so that we won't have to directly change dataframe
        # ? It probably needs some testing
        splitted = options.split("@")

        for option in splitted:
            price_in_option = re.findall(
                r"[(]?[+|-]?\d+(?:,\d{1,3})+?(?:,\d{1,3})?[원]?[)]?", option
            )
            if price_in_option:
                option = option.replace(price_in_option[0], "")
                price = re.sub("[^0-9]", "", price_in_option[0])

                if "|품절" in option:  # Sold out in option
                    sold_out_text = "품절"

                option1 = (
                    option.replace("|품절", "").replace("+\\", "").replace("\\", "")
                )  # option
                option3 = str(int(price))

                try:
                    if "-" in price_in_option:
                        price2 = int(price)
                    else:
                        price2 = int(price2) + int(price)

                except Exception:
                    # case: if price contains string, then we can't add string (price detail) to int (additional price)
                    price2 = int(price)
            else:
                if "|품절" in option:  # option
                    sold_out_text = "품절"

                option1 = option.replace("|품절", "")  # option

            crawl_data = ScubaproCrawlData(
...
async def extract_options(page: PlaywrightPage):
    options1_selectors = await page.query_selector_all(
        "#content > div.pd_r01 > div> div.up_imgp > div.right_txt > div.txt_p > div.pro_btn > select > option"
    )
    ...
    for j in range(len(options1_selectors)):
        ...
        options1_str = "".join(options1_str.split()).replace("|구매가능", "")

        if options1_str != "재고확인":
            final_str += f"{options1_str}@"

    final_str = "".join(final_str.removesuffix("@").split()).replace(" ", "")
...
@cache
def image_quries():
    return "#content > div.pd_r01 div.in_detail img"
