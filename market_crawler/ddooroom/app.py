...
from market_crawler.ddooroom import config
from market_crawler.ddooroom.data import DdooroomCrawlData
...
async def login_button_strategy(page: PlaywrightPage, login_button_query: str):
    ...
    await page.wait_for_selector(
        "#fixed_menu > div > div.xans-element-.xans-layout.xans-layout-statelogon > a.log",
        state="visible",
    )
...
async def extract_soldout_text(product: Element):
    for img in await product.query_selector_all("div.icon > img"):
...
async def extract_product_name(document: Document):
    query = "#contents_main > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table > tbody > tr > td span[style='font-size:12px;color:#292829;font-weight:bold;']"
...
async def extract_table(document: Document | PlaywrightPage, product_url: str):
    ...
    query = "#contents_main > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.infoArea > p.info"
...
async def extract_category_text(document: Document):
    query = "#contents_main > div.xans-element-.xans-product.xans-product-headcategory.path > ol > li:not([class='displaynone'])"
...
def split_options_text(option1: str):
    ...
    if match := compile_regex(r"\(옵션가.*원\)").findall(option1):
        additional_text = match[0]
        option3 = compile_regex(r"\(옵션가(.*)원\)").findall(option1)[0]
        option1 = option1.replace(additional_text, "")

    elif "+" in option1:
        regex = compile_regex(r"\(.*\+\w+[,]?\w*원?\)")
        for additional_text in regex.findall(option1):
            option1 = regex.sub("", option1)
            option3 = compile_regex(r"\(.*\+(\w+[,]?\w*원)\)").findall(additional_text)[
                0
            ]
...
async def extract_images(
    ...
    with suppress(error.PlaywrightTimeoutError):
        await page.click("#tab_box_detail > div > ul > li.detail_here > a")
