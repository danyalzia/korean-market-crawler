...
from market_crawler.nonda import config
from market_crawler.nonda.data import NondaCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "http://nonda.co.kr/")

    categories = await page.query_selector_all("#category > ul > li")
    ...
    for category in categories:
        if (text := await category.text_content()) and "커뮤니티" not in text:
            ...
            subcategories = await page.query_selector_all(
                ".xans-product-menupackage > div > ul > li > a"
            )
...
async def extract_quantity(document: Document):
    query = "#fix_infoArea > div.fix_infoArea_wrap > div.guideArea > p.info"
...
async def extract_table(document: Document | PlaywrightPage):
    ...
    if not (message1 := await document.text_content("#prdDetail > div span")):
        message1 = ""

    return price2, message1.strip()
...
async def extract_html(
    ...
    match await extract_images(page, product_url, html_top, html_bottom):
        ...
        case Err(error.QueryNotFound(err)):
            ...
            return "NOT PRESENT"
        case Err(error.Base64Present(err)):
            await visit_link(page, product_url, wait_until="networkidle")
            match await extract_images(page, product_url, html_top, html_bottom):
                case Ok(images):
                    images = list(dict.fromkeys(images))
