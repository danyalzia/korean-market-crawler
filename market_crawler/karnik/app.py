...
from market_crawler.karnik import config
from market_crawler.karnik.data import KarnikCrawlData
...
async def crawl(
    ...
    if total_pages := await document.get_attribute(
        "#wrap > div.xans-element-.xans-product.xans-product-normalpaging.ec-base-paginate > a.last",
        "href",
    ):
        try:
...
async def get_product_link(product: Element, category_page_url: str):
    if not (el := await product.query_selector("div.thumbnail > div > a")) or not (
...
async def extract_product(
    ...
    match R3:
        case Ok(table):
            (price2, delivery_fee, quantity) = table
        case Err(err):
...
async def extract_product_name(document: Document):
    query = "div.detailArea > div.tab_cate > div.infoArea > div.headingArea > h2"
...
async def extract_table(document: Document):
    ...
    query = "div.detailArea > div.tab_cate > div.infoArea > div.guideArea > p.info"
...
async def extract_images(
    ...
    with contextlib.suppress(error.PlaywrightTimeoutError):
        await page.click(
            "#prdDetail > div.cboth.detail_tabmenu > ul > li:nth-child(1) > a"
        )
