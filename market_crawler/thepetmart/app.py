...
from market_crawler.thepetmart import config
from market_crawler.thepetmart.data import ThepetmartCrawlData
...
async def find_subcategories(browser: PlaywrightBrowser):
    ...
    await visit_link(page, "https://thepetmart.co.kr/")

    categories = await page.query_selector_all("#category > div > ul > li")
    ...
    for category in categories:
        ...
        if category_text not in ["사료(브랜드)", "개인결제창"]:
...
async def extract_url(
    ...
    if options:
        for option in options:
            ...
            crawl_data = ThepetmartCrawlData(
...
async def extract_data(
    ...
    match R3:
        case Ok(table):
            price2, price3, manufacturing_country, manufacturer = table
        case Err(err):
...
async def extract_table(document: Document | PlaywrightPage):
    ...
    query = "#container > div.xans-product-detail > div.detailArea > div.infoArea > div.xans-product-detaildesign > table > tbody"
    ...
    return price2, price3, manufacturing_country, manufacturer
...
async def extract_images(
    ...
    urls: list[str] = []
    for el in elements:
        # sometimes detail images are not present but their html contain urls, so if image urls are visible on page then we extract that urls
        if await el.is_visible() == True:
    ...
    # if urls list is empty then return empty list
    if not urls:
        return urls
