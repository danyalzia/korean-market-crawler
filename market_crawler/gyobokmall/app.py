...
from market_crawler.gyobokmall import config
from market_crawler.gyobokmall.data import GyobokmallCrawlData
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="fieldset > span.button_w1 > a",
...
async def extract_soldout_text(product: Element):
    for icon in await product.query_selector_all("div > div.status > div > img"):
...
async def extract_product(
    ...
    # ? if there are three options, skip the page
    option3_query = "select[id='product_option_id3']"

    if await page.query_selector_all(
        f"{option3_query} > option, {option3_query} > optgroup > option"
    ):
        log.warning(
            f"Three option combo boxes are present ({product_url}), therefore skipping it."
        )
    ...
    if options:
        for option in options:
            ...
            crawl_data = GyobokmallCrawlData(
...
async def extract_data(
    ...
    match R3:
        case Ok(table):
            price2, price3, delivery_fee, message1, quantity = table
        case Err(err):
            await visit_link(page, product_url, wait_until="networkidle")
            R3 = await extract_table(document)  # type: ignore
...
async def extract_product_name(document: Document):
    query = "div.detailArea > div.infoArea > div.xans-element-.xans-product.xans-product-detaildesign > table > tbody > tr > td.name > span"
...
async def extract_table(document: Document | PlaywrightPage):
    ...
    query = "span[class='delv_price_B'] strong"
    ...
    message1 = message1.replace(delivery_fee, "").strip()

    try:
        delivery_fee = parse_int(delivery_fee)
    except ValueError as err:
        raise ValueError(
            f"Coudn't convert delivery fee text {delivery_fee} to number"
        ) from err

    query = "div.detailArea > div.infoArea > div.guideArea > p[class^='info']"
    if not (quantity := await document.text_content(query)):
        raise error.QueryNotFound("Quantity not found", query=query)

    return price2, price3, delivery_fee, message1, quantity
...
def split_options_text(option1: str, price3: int):
    ...
    if "(-￦" in option1:
        ...
        for additional_price in regex.findall(option1):
            option1 = regex.sub("", option1)
            option3 = parse_int(additional_price)
            price3 = price3 - option3

    if "[품절]" in option1:
        return option1.replace("[품절]", "").strip(), "[품절]", option3 or "", price3
    if "품절" in option1:
        return option1.replace("품절", "").strip(), "품절", option3 or "", price3

    return option1.strip(), "", option3 or "", price3
