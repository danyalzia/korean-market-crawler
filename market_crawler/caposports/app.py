...
from market_crawler.caposports import config
from market_crawler.caposports.data import CaposportsCrawlData
...
def get_login_info():
    return LoginInfo(
        ...
        user_id=config.ID,
        password=config.PW,
        user_id_query="div.jl_login input[name='userid']",
        password_query="div.jl_login input[name='passwd']",
        login_button_query="div.jl_login input[type=image][alt='LOGIN']",
...
async def extract_soldout_text(product: Element):
    for img in await product.query_selector_all("li.newproduct_list_in_p > img"):
        if (src := await img.get_attribute("src")) and "soldout" in src:
            return "품절"
...
async def extract_product(
    ...
    if not await page.query_selector(
        "body > main > section > section > section > section.detail-top > div.detail-info > h3"
    ):
        log.warning(f"Product is not longer available: {product_url}")
    ...
    if options1:
        for option1, option2 in zip(options1, options2):
            crawl_data = CaposportsCrawlData(
...
async def extract_product_name(document: Document):
    query = "body > main > section > section > section > section.detail-top > div.detail-info > h3"
...
async def extract_options(document: Document):
    option1_query = "body > main > section > section > section > section.detail-order > form > div.detail-qua > ul > li:nth-child(1)"
    option2_query = "body > main > section > section > section > section.detail-order > form > div.detail-qua > ul > li:nth-child(3)"
    ...
    assert len(option1) == len(option2), "Options1 and Options2 are not equal in length"

    assert (len(option1) == len(option2)) != 0, "Options1 and Options2 are empty"
...
async def extract_table(document: Document, product_url: str):
    query = "div.detail-info span[class='detail-price']"
    ...
    query = "div.detail-info span[class='detail-price2']"
    ...
    model_name = message1 = brand = manufacturing_country = manufacturer = ""

    headings = await document.query_selector_all("ul.detail-info-01 > li")
    values = await document.query_selector_all("ul.detail-info-02 > li")
    ...
    if not model_name:
        log.warning(f"Model name is not present <blue>| {product_url}</>")

    if not message1:
        log.warning(f"Message1 is not present <blue>| {product_url}</>")

    if not brand:
        log.warning(f"Brand country is not present <blue>| {product_url}</>")
...
async def extract_thumbnail_image(document: Document, product_url: str):
    query = "body > main > section > section > section > section.detail-top > div.detail-img > li > img"
...
@cache
def image_quries():
    return "div[class='tab_content'] span[class='product-img'] > img"
...
async def extract_images(
    ...
    with suppress(error.PlaywrightTimeoutError):
        await page.click(
            "body > main > section > section > section > section.detail-bottom > div.detail-tab > ul > li"
        )
