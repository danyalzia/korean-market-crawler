...
from market_crawler.petb2b import config
from market_crawler.petb2b.data import PetB2BCrawlData
...
async def login_button_strategy(page: PlaywrightPage, login_button_query: str):
    ...
    await page.wait_for_selector("xpath=//a[text()='로그아웃']", state="attached")
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="a.loginBtn.-mov",
...
async def crawl(
    ...
    if not (
        total_products_text := await document.text_content(
            "#Product_ListMenu > div > p > strong"
        )
    ):
        raise error.TotalProductsTextNotFound(
            category_url, "Total products text is not found on the page"
        )

    products_len = int(total_products_text)
...
async def extract_product(
    ...
    match await extract_images(page, product_url, html_top, html_bottom):
        case Ok(detailed_images_html_source):
            pass
        case Err(err):
            # ? Some products have base64 that can't be converted back to image format
            # ? See: https://petbtob.co.kr/product/%EB%8F%84%EA%B8%B0%ED%94%84%EB%9E%9C%20%EB%93%9C-%EC%95%BC%EC%B1%84-%EC%9E%A5%EB%82%9C%EA%B0%90-%EA%B5%AC%ED%98%95/4740/category/400/display/1/
...
async def extract_product_name(document: Document):
    query = "h2.item_name, h3.item_name"
...
async def extract_table(document: Document, product_url: str):
    if not (price2 := await document.text_content("#span_product_price_text")):
...
async def extract_options(page: PlaywrightPage):
    ...
    # ? Some options are not displayed, so we need to check it explicitly
    # ? See: https://petbtob.co.kr/product/%EC%95%8C%EB%9F%AC%ED%94%84%EB%9D%BC%EB%8F%84-%EC%95%8C%EB%8F%84%EB%84%9B-%ED%8C%A8%ED%82%A4%EC%A7%80/9795/category/400/display/1/
    option1_query = "select[id='product_option_id1'][class='ProductOption0']"
...
@cache
def image_quries():
    return "#prdDetail > div.cont > p > img, #prdDetail > div.cont > div > img, #prdDetail > div.cont > img, #prdDetail > div.cont > div > p > img, #prdDetail > div.cont > p > span > img"
...
async def extract_images(
    ...
    with suppress(error.PlaywrightTimeoutError):
        await page.click("#prdDetail > div.slide_anchor > ul > li:nth-child(1) > a")
    ...
    for el in elements:
        ...
        match await do(action).retryif(
            ...
            case Ok(src):
                urls.append(src)
            case Err(_):
                # ? Some produts by default have base64 for images that can't be eliminated
                # ? See: https://petbtob.co.kr/product/%EB%8F%84%EA%B8%B0%ED%94%84%EB%9E%9C%20%EB%93%9C-%EC%95%BC%EC%B1%84-%EC%9E%A5%EB%82%9C%EA%B0%90-%EA%B5%AC%ED%98%95/4740/category/400/display/1/
                pass
