...
from market_crawler.accorn import config
from market_crawler.accorn.data import AccornCrawlData
...
def get_login_info():
    return LoginInfo(
        ...
        login_button_query="div.btnLogin > button",
    )
...
async def get_products(document: Document):
    query = "div.xans-element-.xans-product.xans-product-normalpackage > div > ul li[id^='anchorBoxId_']"
...
async def extract_product(
    ...
    match R3:
        case Ok(table):
            (price2, price3, delivery_fee) = table
        case Err(err):
...
async def extract_product_name(document: Document):
    query = "div.prdDescription > div.prdTitle > h2"
...
@cache
def image_quries():
    return "#prdDetailContentLazy img"
