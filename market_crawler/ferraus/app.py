...
from market_crawler.ferraus import config
from market_crawler.ferraus.data import FerrausCrawlData
...
async def run(settings: Settings):
    ...
    login_info = LoginInfo(
        ...
        login_button_query="div.login > fieldset > a > img",
    )
    async with async_playwright() as playwright:
        ...
        if await asyncio.to_thread(os.path.exists, "subcategories.txt"):
            ...
            raise ValueError("Create subcategories.txt manually")
...
async def extract_data(
    ...
    match R3:
        case Ok(table):
            model_name, manufacturer, message1, delivery_fee, price2, price3 = table
        case Err(err):
...
async def extract_table(document: Document):
    model_name = manufacturer = message1 = delivery_fee = ""
    price2 = price3 = 0
    ...
    for th, td in zip(ths, tds):
        ...
        if "상품명" in heading:
            model_name = text
            regex = compile_regex(r"[a-zA-Z0-9\-]+")
            if match := regex.findall(model_name)[-1]:
    ...
    return model_name, manufacturer, message1, delivery_fee, price2, price3
