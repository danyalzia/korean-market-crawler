...
from market_crawler.tnd import config
from market_crawler.tnd.data import TndCrawlData
...
async def get_products(tree: Document):
    query = "div[class='xans-element- xans-product xans-product-listnormal ec-base-product'] li[id^='anchorBoxId_']"
    if not (products := await tree.query_selector_all(query)):
        raise error.QueryNotFound("Products not found", query)

    return products


async def extract_soldout(product: Element):
    if el := await product.query_selector(
        "div.sp-product-spec > div.sp-product-icons > img"
    ):
        if (alt := await el.get_attribute("alt")) and "품절" in alt:
            return "품절"
...
async def extract_product(
    ...
    sold_out_text = await extract_soldout(product)
...
async def extract_data(
    ...
    match R3:
        case Ok(table):
            _, manufacturing_country, price2, price3 = table
        case Err(err):
...
async def extract_product_title(document: Document):
    query = "#contents > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.infoArea > h3"
    if not (product_title := await document.text_content(query)):
        raise error.QueryNotFound("Product title not found", query)
...
async def extract_table(tree: Document):
    product_title = manufacturing_country = ""
    price2 = price3 = 0

    ths = [
        "".join(text.split())
        for t in await tree.query_selector_all(
            "div.sp-detail-baseinfo > table > tbody > tr > th",
        )
    ...
    tds = [
        text.strip()
        for t in await tree.query_selector_all(
            "div.sp-detail-baseinfo > table > tbody > tr > td",
        )
        if (text := await t.text_content())
    ]
    for th, td in zip(ths, tds):
        if th == "상품명":
            ...
            try:
                ...
                raise ValueError(f"Not able to parse selling price text: {td}") from err
        elif th == "원산지":
            manufacturing_country = td

        elif th == "판매가":
            try:
                price2 = parse_int(td)
            except ValueError as err:
                raise ValueError(f"Not able to parse supply price text: {td}") from err

    return Table(product_title, manufacturing_country, price2, price3)


def split_options_text(option1: str, price2: int):
    option3 = ""
    if ":+" in option1 or "(+" in option1:
...
@cache
def image_quries():
    return "#sp-detail-description img"
...
async def extract_images(
    ...
    with suppress(error.PlaywrightTimeoutError):
        await page.click("li.sp-goto[data-target='#prdDetail']", timeout=1000)
...
def is_valid_url(url: str):
    return all(
        image not in url
        for image in ["topnotice_tnd.jpg", "backtitle_tnd_20170516.jpg"]
    )
